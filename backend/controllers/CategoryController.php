...
class CategoryController extends Controller
{
    ...
    {
        $model = new Category();
        $status = [0 => 'Включено', 1 => 'Выключено'];
        $category = ArrayHelper::map(Category::find()->select('id, name')->asArray()->all(), 'id', 'name');
        ...
        if ($model->load(Yii::$app->request->post()) ) {
            ...
            return $this->render('create', compact('model', 'status', 'category'));
        }
    }
